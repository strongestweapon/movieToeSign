...
// events->set_verbose(true); // display on the console what the events send
	...
	// This is possible because movieToeSign inherits from hObject
	// Look at movieToeSign.h to see the implementation

	// Maybe its better to create other objects instead of using movieToeSign
	...
	button2->setMessage("movieToeSign.stop");
	button3->setMessage("movieToeSign.clear");

	// This works because the start, stop and clear listeners are defined in hEvent.cpp
	// and redefined for movieToeSign (see below on the next methods)
	...
	checkbox2->setMessage("movieToeSign.selectItem");
	checkbox3->setMessage("movieToeSign.selectItem");
	checkbox4->setMessage("movieToeSign.selectItem");
	...
	slider4->setMessage("movieToeSign.setValueToItem");
	slider5->setMessage("movieToeSign.setValueToItem");
	slider4->setIndex(1);
	slider5->setIndex(2);
	...
	textBox1->setMessage("movieToeSign.setLabel");
	...
	textArea1->setMessage("movieToeSign.setText");
	textArea1->setMessage2("movieToeSign.clearText");
	...
	listBox1->setMessage("movieToeSign.setLabel");
...
//  hButtonBox(std::string name, hPanel * parent, int dispMode, int xx, int yy, int item_width, int item_height);
	...
	buttonBox1->setMessage("movieToeSign.itemSetSelected");
	...
	textArea2->setMessage("movieToeSign.setText");
	textArea2->setMessage2("movieToeSign.clearText");
	...
	my2Dslider->setMessage("movieToeSign.setXY");
	...
	// Create 2 buttons that will send their messages to a new listener created in movieToeSign:
	...
	events->addListener("setColors", this, &movieToeSign::setColors);
	// "setColors" is the name of the method to call
	// 'this' is the object
	// '&movieToeSign::setColors' is the method

	actionButton1->setMessage("movieToeSign.setColors"); // remember the format
	actionButton2->setMessage("movieToeSign.setColors");

	actionButton1->setIndex(1); // the index is used as parameter by the listener
	actionButton2->setIndex(2);

	// Now you can look at void movieToeSign::setColors(hEventArgs& args)
	// to see how it works
	...
	sliderBox1->setMessage("movieToeSign.setValueToItem");
...
// -------------------------------------------------------------------------------------
	...
	alertDialog->setMessage("movieToeSign.answerDialog");  // Set the message to send when the user answer yes (sends also the value 1)
	alertDialog->setMessage2("movieToeSign.answerDialog"); // Set the message to send when the user answer no  (sends also the value 2)
	...
	actionButton3->setMessage("movieToeSign.openItem");
	actionButton4->setMessage("movieToeSign.openItem");
...
void movieToeSign::start(void){
    ...
    if(!mouseOnMovie){
        if(editMode == ADD ){
            addColumn(0, 0, saveX, saveY, 8,true);
            editMode = DONE;
        }
    }
